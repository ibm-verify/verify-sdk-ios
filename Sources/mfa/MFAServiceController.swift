...
public protocol MFAServiceDescriptor: Actor {
    /// The network session used to coordinate internal data transfer tasks.
    ///
    /// - Warning: **Internal SDK API.** Do not access or modify this property directly.
    /// It is exposed publicly strictly to satisfy protocol conformance and is not part of the public contract. Accessing it may lead to undefined behavior, bypass ephemeral session configurations, or break certificate pinning.
    @_documentation(visibility: private)
    var _urlSession: URLSession { get }
    
    /// The current OAuth access token used for API requests.
    var accessToken: String { get }
    
    /// The location of the endpoint to refresh the OAuth token for the authenticator.
    var refreshUri: URL { get }

    /// The location of the endpoint to perform transaction verification.
    var transactionUri: URL { get }
    
    /// The active transaction currently awaiting user interaction.
    var currentPendingTransaction: PendingTransactionInfo? { get }
    
    /// Refreshes the OAuth token associated with the registered authenticator.
    ///
    /// The resulting access token is automatically updated within the service instance for subsequent operations.
    ///
    /// - Parameters:
    ...
    ///   - pushToken: A token that identifies the device to APNs.
    ///   - additionalData: Optional custom attributes to attach to the refresh request.
    /// - Returns: A new `TokenInfo` containing the refreshed credentials.
    /// - Note: To communicate with the Apple Push Notification service (APNs) and receive a unique device token that identifies your app.  Refer to [Registering Your App with APNs](https://developer.apple.com/documentation/usernotifications/registering_your_app_with_apns).
...
public enum UserAction: String, Sendable {
