...
class CloudAuthenticatorServiceTest: XCTestCase {
    ...
    /// Call nextTransaction by a transaction identifier with correlation code.
    func testNextTransactionWithIDCorrelationCode() async throws {
        // Given
        let transactionId = "b1bd512f-094e-4792-a0f6-6b9c75f50466"
        let allowedCharacterSet = CharacterSet(charactersIn: "\"").inverted
        let queryString = String(format: "\(CloudAuthenticatorService.TransactionFilter.pendingByIdentifier.rawValue)", transactionId)
        ...
        MockURLProtocol.urls[verificationsUrl] = MockHTTPResponse(response: HTTPURLResponse(url: verificationsUrl, statusCode: 200, httpVersion: nil, headerFields: nil)!, fileResource: "cloud.transactionCorrelationCode")
        ...
        XCTAssertEqual(result.countOfPendingTransactions, 2)
        XCTAssertEqual(result.current?.id, transactionId)
        XCTAssertTrue(result.current?.additionalData[.correlation] != nil)
    }
    
    /// Call nextTransaction by a transaction identifier with expiry
    func testNextTransactionExpiryTime() async throws {
        // Given
        let transactionId = "b1bd512f-094e-4792-a0f6-6b9c75f50466"
        let allowedCharacterSet = CharacterSet(charactersIn: "\"").inverted
        let queryString = String(format: "\(CloudAuthenticatorService.TransactionFilter.pendingByIdentifier.rawValue)", transactionId)
        ...
        XCTAssertEqual(result.countOfPendingTransactions, 2)
        XCTAssertEqual(result.current?.id, transactionId)
        XCTAssertTrue(result.current?.expiryTime != result.current?.timeStamp)
    }
